...
<html>
	...
	<body>
		<form action="nav/board.action" id="mainForm" method="get">
				...
				<table class="table table-striped table-bordered table-condensed">
					...
					<tbody>
						<c:forEach items="${boards}" var="board">
							<tr>
								<td><img class="icon" alt="${board.name}" src="image.action?path=${board.icons[0].value}"></td>
								...
								<td>
									...
									<a href="javascript:void(0)" onclick="deleteThis(${board.id})">删除</a>
								</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
				<a class="btn" href="nav/board!input.action">创建板块</a>
			</div>
		</form>
		...
		function deleteThis(id){
			if(confirm("确定要删除吗?")){
				window.location="nav/board!delete.action?id="+id;
			}
		}
		</script>
	</body>
