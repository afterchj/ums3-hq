...
<body>
	<h1>广播列表</h1>
	<form id="mainForm" action="poll/polling.action" method="get" class="form-horizontal">
		...
		<div id="filter" style="margin-bottom:5px;">
			...
			<div class="pull-right"><a href="javascript:void(0)" onclick="goXml()"><i class="icon-rss"></i>XML</a></div>
		</div>
		<table class="table table-striped table-bordered">
			...
			<tbody>
				<s:iterator value="page.result">
					<tr>
						<td><img alt="广告图片" src="${ctx}/files/${previews[0].addr}" style="height: 20px;width: 70px;"></td>
						<td><a href="poll/polling!input.action?id=${id}&pageNo=${page.pageNo}">${name}</a></td>
						<td>
							<s:if test="serType==0">
								上线通知
							</s:if>
						...
						<td><shiro:hasPermission name="store:edit">
							<s:if test="status==0">
								<a href="javascript:void(0)" onclick="changeStatus(${id})">发布</a>
							</s:if><s:elseif test="status==1">
								<a href="javascript:void(0)" onclick="changeStatus(${id})">取消发布</a>
							</s:elseif>	
								<a href="javascript:void(0)" onclick="deleteThis(${id})">删除</a>
		...
		<shiro:hasPermission name="store:edit">
			<a class="btn" id="addpoll" href="javascript:void(0)">添加广播</a>
		</shiro:hasPermission>
	</form>
</body>
</html>