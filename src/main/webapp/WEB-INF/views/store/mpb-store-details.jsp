...
<body>
	<nav class="details">
		<a href="javascript:void(0)" onclick="history.back();return false;"><img
			src="${ctx}/static/images/2.0/back.png">
		</a> 
		<a>${info.title}</a> <a href="store/mpb-store.action?${queryString}"><img
			src="${ctx}/static/images/2.0/home.png">
		</a>
	</nav>
	<section id="container">
		...
		<div class="btns">
			<a
				href="store/mpb-store!details.action?id=${param.id}&offset=${info.offset-1}&${queryString}">
				<img class="offset-r" src="${ctx}/static/images/2.0/offset-left.png">
			</a> <a id="download" href="javascript:void(0)"><span class="more-btn"><img
					src="${ctx}/static/images/2.0/down-split.png"><span
					style="display: block;margin-left: 15px;">免费下载</span>
			</span>
			</a> 
			<a href="store/mpb-store!details.action?id=${param.id}&offset=${info.offset+1}&${queryString}">
		...
		<div class="description soft-info">
			<div class="size introduce J_info">
				<span>大小: <s:if test="info.theme.apkSize!=null">
				...
				<s:if test="info.theme.dtype!=2">
					<span id="down" style="text-align: right"><s:text
							name="total.down" />: ${totalDown}</span>
				</s:if>
				<p>
					<s:text name="home.author" />
					: ${info.author}
				</p>
							...
							function() {
								...
								$
										.ajax({
											...
											dataType : "text",
											data : {
												queryString : '${info.theme.downloadURL}',
												cs : '${queryString}'
											},
											complete : function() {
