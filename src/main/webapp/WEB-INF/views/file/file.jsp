...
<script>
			...
			function deleteThis(id){
				if(confirm("确定要删除吗?")){
					window.location="file/file!delete.action?id="+id+"&dtype="+'${param['filter_EQS_dtype']}';
				}
			}
			
			function doupload(){
				window.location="file/file-upload.action?dtype="+'${param['filter_EQS_dtype']}';
			}
            function doHot(id,pageNo,type){
                window.location="file/file!hotTag.action?id="+id+"&page.pageNo="+pageNo+"&dtype="+type;
            }
            function doNew(id,pageNo,type){
               window.location="file/file!newTag.action?id="+id+"&page.pageNo="+pageNo+"&dtype="+type;
            }
		</script>
</head>
<body>
	<h1>文件列表</h1>
	<form id="mainForm" action="file/file.action" method="post"
		...
		<ul class="nav nav-pills" style="margin-bottom: 2px; float: left;">
			<li id="standard"><a href="file/file.action?filter_EQS_dtype=0">标准解锁</a>
			</li>
			<li id="model"><a href="file/file.action?filter_EQS_dtype=1">DIY解锁</a>
			</li>
			<li id="app"><a href="file/file.action?filter_EQS_dtype=2">应用</a>
			</li>
		</ul>
		<div id="filter" style="display: inline; margin-left: 20px;">
			<input type="text" class="search-query input-medium"
				...
				size="20" placeholder="标题" /> <input type="button" id="submit_btn"
				...
				<s:iterator value="page.result">
					<tr>
						<td><s:if test="ishot==1">
							...
							href="file/file!input.action?dtype=${dtype}&id=${id}&page.pageNo=${page.pageNo}">${title}</a>
						</td>
						<td>${name}</td>
						<td>${version}</td>
						<td>${tagNames}</td>
						...
						<td><shiro:hasPermission name="file:edit">
								...
								<s:if test="isnew==1">
									<a href="javascript:void(0)"
										onclick="doNew('${id}','${page.pageNo}','${dtype}')"><i
										class="icon-star"></i></a>
								</s:if>
								<s:else>
									<a href="javascript:void(0)"
                            ...
                            <s:if test="ishot==1">
									<a href="javascript:void(0)"
										onclick="doHot('${id}','${page.pageNo}','${dtype}')"><i
