		...
		<script>
			...
			function deleteThis(id){
				if(confirm("该操作会删除所有关联货架!确定要删除吗?")){
					window.location="category/store!delete.action?id="+id;
				}
			}
		</script>
	</head>
	<body>
		<form action="category/store.action" method="get">
			<h1>商店列表</h1>
			...
			<div class="pull-right">
                <shiro:hasPermission name="store:edit">
                    <a class="icon-plus" href="category/store!input.action">新增</a>
                </shiro:hasPermission>
			</div>
			<table class="table table-striped table-bordered">
				...
				<s:iterator value="stores">
					<tr>
						...
						<td>
							<shiro:hasPermission name="store:edit">
								<a href="category/store!input.action?copyId=${id}"><i class="icon-copy"></i></a>&nbsp;
								<a href="category/store!input.action?id=${id}"><i class="icon-edit"></i></a>&nbsp;
