...
import com.tp.utils.*;
import com.tp.web.vo.Page;
import com.tp.web.vo.Page.Sort;
import com.tp.web.vo.PropertyFilter;
import com.tp.web.vo.PropertyFilter.MatchType;
...
public abstract class BaseHibernateDaoImpl<E, I extends Serializable> implements BaseDao<E, I> {
	...
	public Criteria createCriteria(final Criterion... criterions) {
		return createCriteria(Lists.newArrayList(criterions));
	}
	...
	@Override
	public List<E> findByConditionWithPage(List<PropertyFilter> filters, Page page) {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<E> findByCriterions(final List<Criterion> criterions, List<Order>orders) {
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<E> findByCriterions(final List<Criterion> criterions, List<Order>orders, Page page) {
		Criteria criteria = createCriteria(criterions);
		if(CollectionUtils.isNotEmpty(orders)){
			for(Order order : orders){
				criteria.addOrder(order);
	...
	public List<E> findByHql(final String hql, final List<Object> params) {
		if (CollectionUtil.isBlank(params)) {
			return createQuery(hql, ArrayUtils.EMPTY_OBJECT_ARRAY).list();
		}
		return createQuery(hql, params.toArray()).list();
	}
	...
	public List<E> findByHql(final String hql, final List<Object> params, 	final int startIndex, final int count) {
		Query q = null;
		if (CollectionUtil.isBlank(params)) {
			q = createQuery(hql, ArrayUtils.EMPTY_OBJECT_ARRAY);
		} else {
			q = createQuery(hql, params.toArray());
		}
	...
	@SuppressWarnings("unchecked")
	private List<E> findPage(Page page, Criterion[] criterions) {
		if (page == null || page.getStartRow() == null) {
...
*/
	...
	@Override
	@Deprecated
	public Long getTotalCount(final String hql, final List<Object> params) {
		Query q = getCurrentSession().createQuery(hql);
		for (int i = 0; i < params.size(); i++) {
			q.setParameter(i, params.get(i));
		}
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<E> list(Integer status) {
		if(status == null){
			return list();
		}
	...
	private Criteria setPageRequestToCriteria(Criteria c, Page page) {
		...
		if (page.isOrderBySetted()) {
			for (Sort sort : page.getSort()) {
				if (Sort.ASC.equals(sort.getValue())) {
					c.addOrder(Order.asc(sort.getKey()));
				} else {
					c.addOrder(Order.desc(sort.getKey()));
				}
			}
		}
		return c;
	...
	@Override
	public List<E> findWithPage(Page page) {
		return findByHql("from " + entityClass.getName(), null, page.getStartRow(), page.getPageSize());
	}
